...
<body id="appBody" class="login-body">
  ...
  <div id="loginView" class="container">
    ...
    <div class="login-container">
      ...
      <div class="links">
        <a href="http://51.21.167.202:8013/">Sign Up</a> | <a href="http://51.21.167.202:8010">Forgot Password?</a>
      </div>
    </div>
  </div>
  ...
  <div id="dashboardView" class="hidden">
    ...
    <main class="main-content" id="mainContent">
      <iframe id="contentFrame" src="http://51.21.167.202:7999/" loading="lazy"></iframe>
    </main>
...
document.addEventListener('DOMContentLoaded', async () => {
  const config = {
    apiBaseUrl: 'http://51.21.167.202:3404/api',
    wsUrl: 'ws://51.21.167.202:4500',
    authUrls: {
      login: 'http://51.21.167.202:8012',
      signup: 'http://51.21.167.202:8013/',
      forgotPassword: 'http://51.21.167.202:8010'
    },
    modules: {
      emp_attendance: 'http://51.21.167.202:8051/',
      emp_leave: 'leave.html',
      emp_wfh: 'http://51.21.167.202:8025/',
      emp_payslip: 'http://51.21.167.202:7019/',
      emp_tasks: 'http://51.21.167.202:8045/',
      emp_helpdesk: 'http://51.21.167.202:8049/',
      emp_Onboarding: 'http://51.21.167.202:8039/',
      emp_benefits: 'http://51.21.167.202:8043/',
      emp_Appraisal: 'http://51.21.167.202:8014/',
      emp_notifications: 'http://51.21.167.202:8053/',
      emp_asset: 'http://51.21.167.202:8047/',
      emp_bonus: 'http://51.21.167.202:8055/',
      emp_joblists: 'http://51.21.167.202:8020/',
      emp_claim: 'http://51.21.167.202:8027/',
      emp_offboarding: 'http://51.21.167.202:8041/',
      emp_jobapplication: 'http://51.21.167.202:8031/',
      emp_offerletter: 'http://51.21.167.202:8033/',
      emp_logout: '',
      hr_attendance: 'http://51.21.167.202:8052/',
      hr_leave: 'leave.html',
      hr_wfh: 'http://51.21.167.202:8026/',
      hr_payslip: 'http://51.21.167.202:7020/',
      hr_tasks: 'http://51.21.167.202:8046/',
      hr_helpdesk: 'http://51.21.167.202:8050/',
      hr_Onboarding: 'http://51.21.167.202:8040/',
      hr_employeemanagement: 'http://51.21.167.202:8036/',
      hr_benefits: 'http://51.21.167.202:8044/',
      hr_appraisal: 'http://51.21.167.202:8015/',
      hr_notifications: 'http://51.21.167.202:8054/',
      hr_asset: 'http://51.21.167.202:8048/',
      hr_bonus: 'http://51.21.167.202:8056/',
      hr_joblists: 'http://51.21.167.202:8021/',
      hr_claim: 'http://51.21.167.202:8028/',
      hr_offboarding: 'http://51.21.167.202:8042/',
      hr_jobapplication: 'http://51.21.167.202:8032/',
      hr_logout: ''
    },
  ...
  const utils = {
    ...
    initWebSocket: () => {
      ...
      state.ws.onopen = () => {
        ...
        utils.logToServer('WebSocket connected', 'info');
      };

      state.ws.onmessage = (event) => {
        try {
          const data = JSON.parse(event.data);
          if (data.type === 'EMP_DETAILS') {
            localStorage.setItem('abccompanyempdetails', JSON.stringify(data.data));
            state.personnelDetails = data.data;
            utils.showAlert('success', 'Employee details received and saved');
            utils.logToServer('Received and saved employee details via WebSocket', 'info');
            core.autoPopulateFields(data.data);
          }
        } catch (err) {
          console.error('WebSocket message error:', err);
          utils.logToServer(`WebSocket message error: ${err.message}`, 'error');
        }
      };

      state.ws.onclose = () => {
        console.warn('WebSocket disconnected');
        setTimeout(utils.initWebSocket, 5000);
      };

      state.ws.onerror = (err) => {
        console.error('WebSocket error:', err);
        utils.logToServer(`WebSocket error: ${err.message}`, 'error');
      };
    }
  };
  ...
  const core = {
    ...
    autoPopulateFields: (empData) => {
      const fields = {
        'emp-name': empData.name,
        'emp-email': empData.email,
        'emp-id': empData.emp_id
      };
